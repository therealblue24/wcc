#include "bird.h"
#include "ir.h"
#include "zz/list.h"

static uint64_t hashd(uint64_t x)
{
	return fnv1a(&x, sizeof(uint64_t));
}

static uint64_t hashk(uint64_t x, uint64_t k)
{
	uint64_t v;
	v = hashd(x << ((k ^ x) & 3));
	v ^= k;
	return fnv1a(&v, sizeof(uint64_t));
}

static uint64_t hashp(void *x)
{
	return fnv1a(&x, sizeof(uint64_t));
}

static int ins_has_imm(enum ins_type t)
{
	return t == IR_INST_IMM || t == IR_INST_LEAS || t == IR_INST_LOADS ||
		   t == IR_INST_LOADSS || t == IR_INST_STORES || t == IR_INST_STORESS;
}

static bool do_not_gvn_type(enum ins_type t)
{
	return t == IR_INST_LOAD || t == IR_INST_LOADS || t == IR_INST_STORE ||
		   t == IR_INST_STORES || t == IR_INST_CALL || t == IR_INST_PHI ||
		   ir_inst_is_term(t);
}

static bool do_not_gvn(ir_inst_t *i)
{
	if(!i->r0 || do_not_gvn_type(i->type) || i->noopt) {
		return true;
	}

	if(i->r0->phi_related) {
		return true;
	}

	return false;
}

/* excludes ins->r0; assumes instruction is GVN-able */
static uint64_t hash_ins(ir_inst_t *ins)
{
	uint64_t hash = hashd(ins->type);

	/* I had some fun with these constants. They could be
	 * anything random enough, really. */
	hash ^= hashk(ins->sign_ext, 0x1618033988749894);
	hash ^= hashk(ins->size, 0x2718281828459045);
	hash ^= hashk(ins->is_32bit, 0x3141592653589793);

	uint64_t imm = ins->imm;
	if(ins->is_32bit && ins->type == IR_INST_IMM) {
		imm &= UINT32_MAX;
	}

	if(ins_has_imm(ins->type))
		hash ^= hashd(imm);
	if(ir_find(ins->r1))
		hash ^= hashp(ir_find(ins->r1));
	if(ir_find(ins->r2))
		hash ^= hashp(ir_find(ins->r2));
	if(ins->type == IR_INST_LEA)
		hash ^= hashp(ins->label);
	return hash;
}

static bool ins_are_same(ir_inst_t *a, ir_inst_t *b)
{
	if(!a || !b) {
		return false;
	}

	if(a == b) {
		return true;
	}

	if(a->type != b->type) {
		return false;
	}

	if(a->type == IR_INST_IMM) {
		uint64_t imm1 = a->imm;
		uint64_t imm2 = b->imm;
		if(a->is_32bit)
			imm1 &= UINT32_MAX;
		if(b->is_32bit)
			imm2 &= UINT32_MAX;
		if(imm1 != imm2) {
			return false;
		} else {
			return true;
		}
	}

	if(a->sign_ext != b->sign_ext || a->size != b->size ||
	   a->is_32bit != b->is_32bit || ir_find(a->r1) != ir_find(b->r1) ||
	   ir_find(a->r2) != ir_find(b->r2)) {
		return false;
	}

	if(ins_has_imm(a->type) && a->imm != b->imm) {
		return false;
	}

	if(a->type == IR_INST_LEA && a->label != b->label) {
		return false;
	}

	return true;
}

typedef struct lookup {
	size_t loc;
	bool exists;
} lookup_t;

static lookup_t lookup_map(ir_inst_t **map, size_t cap, ir_inst_t *ins)
{
	uint64_t hash = hash_ins(ins);
	uint64_t mask = cap - 1;
	hash &= mask;
	uint64_t step = 1; /* TODO: improve */
	for(uint64_t i = hash - step;;) {
		i = (i + step) & mask;

		if(map[i] == NULL) {
			return (lookup_t){ .loc = i, .exists = false };
		}

		/* check if it's really it */
		if(ins_are_same(ins, map[i])) {
			/* yup */
			return (lookup_t){ .loc = i, .exists = true };
		}
	}

	/* you don't get here */
	return (lookup_t){ .loc = -1, .exists = false };
}

static ir_inst_t **rebuild_map(ir_inst_t **map, size_t *cap);

static ir_inst_t **insert_map(ir_inst_t **map, size_t *cap, size_t *keys,
							  ir_inst_t *ins)
{
	lookup_t l = lookup_map(map, *cap, ins);
	if(!l.exists) {
		(*keys)++;
	}

	if(*keys >= (3 * *cap) / 4) {
		map = rebuild_map(map, cap);
	}

	map[l.loc] = ins;
	return map;
}

static ir_inst_t *find(ir_inst_t **map, size_t cap, ir_inst_t *ins)
{
	lookup_t l = lookup_map(map, cap, ins);
	return l.exists ? map[l.loc] : NULL;
}

static ir_inst_t **rebuild_map(ir_inst_t **map, size_t *cap)
{
	ir_inst_t **new = zcalloc(*cap * 2, sizeof(ir_inst_t *));

	size_t keys = 0;
	size_t newcap = *cap * 2;
	for(size_t i = 0; i < *cap; i++) {
		if(!map[i]) {
			continue;
		}

		insert_map(new, &newcap, &keys, map[i]);
	}
	*cap = newcap;

	free(map);

	return new;
}

static int gvn_block(ir_blk_t *blk)
{
	/* entry block */
	if(blk->idom == blk) {
		blk->gvn_cap = GVN_INIT_SIZE;
		blk->gvn_keys = 0;
		blk->gvn_map = zcalloc(blk->gvn_cap, sizeof(ir_inst_t *));
	} else {
		/* inherit from dominator */
		blk->gvn_cap = blk->idom->gvn_cap;
		blk->gvn_keys = blk->idom->gvn_keys;
		blk->gvn_map =
			memdup(blk->idom->gvn_map, blk->gvn_cap * sizeof(ir_inst_t *));
	}

	int change = 0;
	for(ir_inst_t *ins = blk->insts; ins; ins = ins->next) {
		if(!ins->r0) {
			continue;
		}
		if(do_not_gvn(ins)) {
			continue;
		}
		ir_inst_t *found = find(blk->gvn_map, blk->gvn_cap, ins);
		if(found) {
			ins->type = IR_INST_NOP;
			ir_union(ins->r0, found->r0);
			ins->r0 = ins->r1 = ins->r2 = NULL;
			change = 1;
		} else {
			/* welcome to GVN */
			blk->gvn_map =
				insert_map(blk->gvn_map, &blk->gvn_cap, &blk->gvn_keys, ins);
		}
	}
	return change;
}

/* gvn dfs visit of dominator tree */
static int dfs_visit(ir_blk_t *blk)
{
	int change = gvn_block(blk);
	for(size_t i = 0; i < list_len(blk->dom); i++) {
		change |= dfs_visit(blk->dom[i]);
	}
	return change;
}

int ir_gvn(ir_func_t *func)
{
	int change = 0;

	ir_fix(func);
	dfs_visit(func->blocks[0]);

	/* cleanup */
	for(size_t i = 0; i < list_len(func->blocks); i++) {
		free(func->blocks[i]->gvn_map);
		func->blocks[i]->gvn_map = NULL;
	}

	ir_rewrite(func);

	return change;
}
